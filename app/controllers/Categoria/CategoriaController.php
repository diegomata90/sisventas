...
class CategoriaController extends Controller
{
  ...
  public function Agregar($request_params)
  {
    ...
    if(!$result || !$this->model->affected_rows())
      return $this->New('Hubo un error al agregar la Categoria');
    
    return $this->Listar('Categoria Agregada!!');
  }
  ...
  {
      if(empty($id))
        return $this->Listar('No se recibió el ID del Categoria', 'warning');

      if(!is_numeric($id))
        return $this->Listar('El ID del Categoria debe ser numérico', 'warning');

      $result = $this->model->Eliminar($id);

      if(!$result || !$this->model->affected_rows())
        return $this->Listar("Hubo un error al remover el Categoria número {$id}", 'warning');
  ...
  public function Actualizar($request_params)
  {
    ...
    if(!is_numeric($request_params['id']))
      return $this->Listar('El ID del Categoria debe ser numérico para editar', 'warning');
    
    $result = $this->model->Actualizar($request_params);  

    if(!$result || !$this->model->affected_rows())
      return $this->Listar("Hubo un error al editar el Categoria número {$request_params['id']}", 'warning');

    $this->Listar("Categoria número {$request_params['id']} actualizado");
  }
